						       ...
						       const char *cipher_name)
{
    ...
    printk("sun-ccm: allocated and ready\n");
...
static struct crypto_instance *crypto_ccm_alloc(struct rtattr **tb)
{
	...
	if (snprintf(ctr_name, CRYPTO_MAX_ALG_NAME, "sun-ctr(%s)",
		     cipher_name) >= CRYPTO_MAX_ALG_NAME)
		return ERR_PTR(-ENAMETOOLONG);

	if (snprintf(full_name, CRYPTO_MAX_ALG_NAME, "sun-ccm(%s)", cipher_name) >=
	    CRYPTO_MAX_ALG_NAME)
		return ERR_PTR(-ENAMETOOLONG);
...
static struct crypto_instance *crypto_ccm_base_alloc(struct rtattr **tb)
{
	...
	if (snprintf(full_name, CRYPTO_MAX_ALG_NAME, "sun-ccm_base(%s,%s)",
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Sun Compatible Counter with CBC MAC");
MODULE_ALIAS("sun-ccm_base");
