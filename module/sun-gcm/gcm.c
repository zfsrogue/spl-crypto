...
static struct crypto_instance *crypto_gcm_alloc(struct rtattr **tb)
{
	...
	if (snprintf(ctr_name, CRYPTO_MAX_ALG_NAME, "sun-ctr(%s)", cipher_name) >=
	    CRYPTO_MAX_ALG_NAME)
		return ERR_PTR(-ENAMETOOLONG);

	if (snprintf(full_name, CRYPTO_MAX_ALG_NAME, "sun-gcm(%s)", cipher_name) >=
	    CRYPTO_MAX_ALG_NAME)
		return ERR_PTR(-ENAMETOOLONG);
...
static struct crypto_instance *crypto_gcm_base_alloc(struct rtattr **tb)
{
	...
	if (snprintf(full_name, CRYPTO_MAX_ALG_NAME, "sun-gcm_base(%s,%s)",
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Solaris Compatible Galois/Counter Mode");
MODULE_AUTHOR("Mikko Herranen <mh1@iki.fi>");
MODULE_ALIAS("sun-gcm_base");
